...
void ggml_cuda_flash_attn_ext_tq_mse(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    // Step 1: rotate Q once — q_pi = scale * Pi @ Q
    ggml_cuda_pool_alloc<float> q_pi_buf(ctx.pool(), n_queries * D);
    {
        ...
        tq_mse_rotate_q_kernel<D><<<grid_rot, block_rot, smem_rot, ctx.stream()>>>(
            (const char *) Q->data,
            q_pi_buf.ptr,
            tq_d_Pi_T[dim_idx],
            scale,
    ...
    // Step 2: split-K partial attention (accumulates in Pi-rotated domain)
    ...
    // warp-parallel layout: [NWARPS*D acc_rot | 3*NWARPS softmax scalars | 2 global scalars]
    ...
    // Step 3: combine splits + single Pi^T unrotation
